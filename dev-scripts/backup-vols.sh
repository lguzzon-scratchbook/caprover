...
usage() {
  echo >&2 "Usage: backup-vols.sh <backup|restore>"
  exit 1
}
...
backup() {
  ...
  for VOL_NAME_TO_BACKUP in $vols; do
...
restore() {
  ...
  if [ -z "$(ls -A /$restore_dir)" ]; then
    echo >&2 "$restore_dir is empty or missing, check if you specified a correct name"
    exit 1
  fi
